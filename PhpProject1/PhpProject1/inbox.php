<?php include "base.php"; include "query.php" ;include "getQuery.php" ?>

<?php
    appendMenuBar(false);

    $action = $_SESSION["user_name"] . " viewed the message inbox.";
    ...
    for($i = 0; $i < $elements_length; $i++)
    {
        ...
        for($j = 2; $j< $keys_length; $j++)
        {
            ...
            elseif ($keys[$j] == "USER_NAME")
                $content .= "<th style='border-style: solid; border-width: 1px; text-align: left; padding: 5px'><a href='user_details.php?user_id=" . $messages[1][$i][0] . "&page_name=inbox&previous_page=inbox.php'>" . $messages[1][$i][2] . "</a></th>";
            else
        ...
        $content .= "<input type='button' value='Reply' onclick=location.href='reply_message.php?id=" . $to_user . "&previous_page=inbox.php'>";
        $content .= "<input type='button' value='Details' onclick=location.href='message_details.php?message_id=" . $message_id . "&previous_page=inbox.php'>";
        $content .= "<input type='button' value='Delete' onclick=location.href='delete.php?page_name=inbox.php&table_name=MESSAGE&id_name=MESSAGE_ID&id=" . $message_id . "'>";
        $content .= "</th>";
        $content .= "</tr>";
    }
